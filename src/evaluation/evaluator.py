"""
Evaluator â€” evaluation.evaluator
...
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # â†’ src/
...
def smooth_mae(mae: np.ndarray, window: int = 3) -> np.ndarray:
    """Apply a short moving average to an MAE signal."""
    if len(mae) <= window:
        return mae
...
class LSTMEvaluator:
    ...
    def evaluate_train_val(self, model, X_train, y_train, X_val, y_val) -> dict:
        ...
        for name, X, y in [("train", X_train, y_train), ("val", X_val, y_val)]:
            ...
            print(
                f"  {name.capitalize()}: "
                f"MSE={errors[name]['mean_mse']:.6f} Â± {errors[name]['std_mse']:.6f}  |  "
                f"MAE={errors[name]['mean_mae']:.6f} Â± {errors[name]['std_mae']:.6f}"
            )
        return errors
    ...
    def evaluate_per_asset(self, asset_filter: list = None, use_adaptive: bool = False) -> None:
        ...
        print("=" * 100)
        print("LSTM â€” Per-Asset Test Set Evaluation")
        print("=" * 100)
        ...
        for asset_dir in asset_dirs:
            ...
            print(f"  Asset {asset_id} â€” Auto Threshold (val p85) = {threshold:.5f}")
...
class TreeEvaluator:
    ...
    def evaluate_event_level(
        ...
        print(f"\n{'='*100}")
        print(f"{model_name} â€” Event-Level Detection (threshold={threshold:.4f})")
        print(f"{'='*100}")
        print(
    ...
    def evaluate_per_asset(self, model_name: str, use_stats: bool = False, use_adaptive: bool = False) -> None:
        ...
        for asset_dir in asset_dirs:
            ...
            print(f"\n  Asset {asset_id} â€” model loaded (threshold={threshold:.4f})")
