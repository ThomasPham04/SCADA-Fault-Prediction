"""
02_tune_autoencoder_threshold.py
=============================================================
Phase 2: Ultra-fast Tuning Phase.
Loads the pre-trained models, applies a user-defined threshold,
and evaluates prediction splits to draw the Confusion Matrix.

Usage:
    python experiments/02_tune_autoencoder_threshold.py --threshold 85
=============================================================
"""
...
RESULTS_OUT    = os.path.join(RESULTS_DIR, "autoencoder_batch")
...
def plot_confusion_matrix(tp, fp, fn, tn, acc, prec, rec, f1, threshold_pct, anomaly_ratio, out_path):
    ...
    ax.set_title(f"AutoEncoder CM (Threshold: {threshold_pct}th Pct | Anomaly Ratio: {anomaly_ratio})", color="white", fontsize=12)
...
def main():
    ...
    print("="*60)
    print(f"PHASE 2: TUNING & EVAL (Threshold = {args.threshold}th Pct)")
    print("="*60)
    ...
    for csv in csvs:
        ...
        if not all(os.path.exists(p) for p in [model_path, scaler_path, mse_train_path, feat_cols_path]):
            print(f"[{eid}] SKIP: Missing trained assets. Run Phase 1 first.")
            continue
    ...
    cm_path = os.path.join(RESULTS_OUT, f"tuning_matrix_th{args.threshold}_ar{args.anomaly_ratio}.png")
